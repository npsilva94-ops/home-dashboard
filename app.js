...
var menuDirty={};
function renderMenu(){
 var box=el("weeklyMenu"),i,key,val,ta;
 if(!box)return;
 if(!box.getAttribute("data-ready")){
  var o="";
  for(i=0;i<menuDays.length;i++){
   key=menuDays[i][0];val=state.menu[key]||"";
   o+='<div class="menuDay"><b>'+menuDays[i][1]+'</b><textarea data-menu="'+key+'" placeholder="Ementa…">'+esc(val)+'</textarea><button type="button" class="menuSave" data-save-menu="'+key+'">Guardar</button></div>'
  }
  box.innerHTML=o;
 ...
 for(i=0;i<menuDays.length;i++){
  key=menuDays[i][0];
  if(menuDirty[key])continue;
  ta=box.querySelector('textarea[data-menu="'+key+'"]');
  if(ta){
   val=state.menu[key]||"";
...
function toggleSectionExpand(kind,btn){
 var card=null;
 if(kind==="events")card=document.querySelector(".card.agenda");
 if(kind==="shopping")card=document.querySelector(".card.shopping");
 if(kind==="tasks")card=document.querySelector(".card.tasks");
 if(!card)return;
...
var EXPENSE_CATS={credit:{name:"Crédito",cls:"cat-credit",color:"#4F7CAC"},supermarket:{name:"Supermercado",cls:"cat-supermarket",color:"#4FA66A"},house:{name:"Casa",cls:"cat-house",color:"#E0B43C"},car:{name:"Carro",cls:"cat-car",color:"#E67E22"},health:{name:"Saúde",cls:"cat-health",color:"#D94F70"},gym:{name:"Ginásio",cls:"cat-gym",color:"#7B61B3"},leisure:{name:"Lazer",cls:"cat-leisure",color:"#20A4B8"},restaurant:{name:"Restauração",cls:"cat-restaurant",color:"#C7653D"},other:{name:"Outros",cls:"cat-other",color:"#7D858C"}};
